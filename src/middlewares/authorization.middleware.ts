...
export const checkWorkspaceRole = (requiredRole: WorkspaceRole) => {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      const currentUserWorkspaceRoleLevel = workspaceRoleHierarchy[userOnWorkspace.role];
      const requiredRoleLevel = workspaceRoleHierarchy[requiredRole];

      const isWorkspaceOwner = workspace.ownerId === userId;

      if (requiredRole === WorkspaceRole.OWNER && !isWorkspaceOwner) {
      ...
      return next(new UnauthorizedError("Insufficient workspace privileges."));
    } catch (error) {
