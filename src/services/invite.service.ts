...
export async function createInvite(inviteData: CreateInviteSchemaType, user: IUser) {

    const workspace = await prisma.workspace.findUnique({
        where: {
            id: inviteData.workspaceId
        }
    });
    if (!workspace) {
...
export async function acceptInvite(inviteDate: acceptInviteSchemaType, user: IUser) {
    const invite = await prisma.invite.findUnique({
        where: {
            token: inviteDate.token
        }
    })
